...
class PPOLIP:
    ...
    def _calc_grad_penalty(self, obs_batch, actions_log_prob_batch,original_batch_size):
        grad_log_prob = torch.autograd.grad(actions_log_prob_batch.sum(), obs_batch, create_graph=True)[0]
        
        gradient_penalty_loss = torch.sum(torch.square(grad_log_prob[:original_batch_size]), dim=-1).mean()
    ...
    def update(self):  # noqa: C901
        ...
        for (
            ...
            #print("obs_est_batch", obs_est_batch)
            ...
            gradient_penalty_loss = self._calc_grad_penalty(obs_est_batch, actions_log_prob_batch, original_batch_size)
            ...
            loss += gradient_penalty_loss*self.grad_penalty_coef
            ...
            # print('obs est gradient before', obs_est_batch.grad)
            # print('obs grad before', obs_batch.grad)
            loss.backward()
            # print('obs est gradient after', obs_est_batch.grad)
            # print('obs grad after', obs_batch.grad)
            # -- For RND
            if self.rnd:
