...
def terrain_levels_target(
    env: ManagerBasedRLEnv, env_ids: Sequence[int],target_pos: tuple[float, float, float], reached_distance: float, minimal_covered: float,asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    ...
    asset: Articulation = env.scene[asset_cfg.name]
    terrain: TerrainImporter = env.scene.terrain
    target_pos_e =  torch.tensor(target_pos, device=env.device)
