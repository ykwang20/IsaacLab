...
class G1Rewards:
    ...
    alive_reward=RewTerm(func=mdp.is_alive, weight=2)#100
...
class CommandsCfg:
    ...
    base_velocity = mdp.UniformVelocityCommandCfg(
        ...
        ranges=mdp.UniformVelocityCommandCfg.Ranges(
            lin_vel_x=(0.2, 1.0), lin_vel_y=(-0., 0.), ang_vel_z=(0.0, 0.0), heading=(-0.*math.pi, 0.*math.pi)
        ),
    )
...
class CurriculumCfg:
    ...
    terrain_levels = CurrTerm(func=mdp.terrain_levels_box_vel)
...
@configclass
class G1VelEnvCfg(LocomotionVelocityRoughEnvCfg):
...
@configclass
class G1VelEnvCfg_Play(G1VelEnvCfg):
    def __post_init__(self):
        # post init of parent
