...
class MotorGroup:
  ...
  def __init__(self,
    ...
    self._strength_ratios = torch.ones((self._num_envs, self._num_motors),
                                       device=device)
    self._init_positions = convert_to_torch([motor._init_position for motor in motors],
                                    device=device)
    self._min_positions = convert_to_torch([motor._min_position for motor in motors],
                                   device=device)
    self._max_positions = convert_to_torch([motor._max_position for motor in motors],
                                   device=device)
    self._min_velocities = convert_to_torch([motor._min_velocity for motor in motors],
                                    device=device)
    self._max_velocities = convert_to_torch([motor._max_velocity for motor in motors],
                                    device=device)
    self._min_torques = convert_to_torch([motor._min_torque for motor in motors],
                                 device=device)
    self._max_torques = convert_to_torch([motor._max_torque for motor in motors],
                                 device=device)
    self._kps = convert_to_torch([motor._kp for motor in motors], device=device)
    self._kds = convert_to_torch([motor._kd for motor in motors], device=device)
  ...
  def strength_ratios(self, value: _FloatOrArray):
    self._strength_ratios = torch.ones(
        self._num_motors, device=self._device) * convert_to_torch(value,
                                                          device=self._device)
