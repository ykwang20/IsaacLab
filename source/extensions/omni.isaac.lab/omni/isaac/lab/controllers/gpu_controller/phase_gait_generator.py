...
class PhaseGaitGenerator:
  ...
  def __init__(self, robot: BaseRobot, gait_config: GaitConfig):
    ...
    self._initial_offset = convert_to_torch(gait_config.initial_offset,
                                           device=self._device)
    self._swing_ratio = convert_to_torch(gait_config.swing_ratio,
                                        device=self._device)
    self._stepping_frequency = gait_config.stepping_frequency
    self.reset()

  def reset(self):
    self._current_phase = torch.stack([self._initial_offset] *
                                      self._num_envs,
                                      axis=0).to(self._device)
    self._stepping_frequency = torch.ones(
        self._num_envs, device=self._device) * self._stepping_frequency
    self._swing_cutoff = torch.ones(
        (self._num_envs, 4),
        device=self._device) * 2 * torch.pi * (1 - self._swing_ratio)
