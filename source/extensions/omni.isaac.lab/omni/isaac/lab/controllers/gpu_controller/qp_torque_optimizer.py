...
from .rotation_utils import quat_to_rot_mat, quat_mul, quat_from_euler_xyz, quat_rotate
...
def compute_desired_acc(
  ...
  desired_lin_acc_body_frame = torch.matmul(
      base_rot_mat_t.float(), (desired_lin_acc_gravity_frame[:, :, None]).float())[:, :, 0]
  desired_ang_acc_body_frame = torch.matmul(
      base_rot_mat_t.float(), (desired_ang_acc_gravity_frame[:, :, None]).float())[:, :, 0]
  
  # desired_lin_acc_body_frame = torch.matmul(
  #     base_rot_mat_t.double(), desired_lin_acc_gravity_frame[:, :, None])[:, :, 0]
  # desired_ang_acc_body_frame = torch.matmul(
  #     base_rot_mat_t.double(), desired_ang_acc_gravity_frame[:, :, None])[:, :, 0]
  
  indices = torch.tensor([0], device=base_position.device)
  # print(f"Base rpy: {base_rpy[indices]}")
  # print(f"Base quat: {base_quat[indices]}")
  # print(f"desired_base_orientation_rpy: {desired_base_orientation_rpy[indices]}")
  #print(f"Desired position: {desired_base_position[indices]}")
  #print(f"Current position: {base_position[indices]}")
  #print(f"linear pos error: {lin_pos_error[indices]}")
  #print(f"linear vel error: {lin_vel_error[indices]}")
  #print(f"angular pos error: {ang_pos_error[indices]}")
  

  # print(f"desired_angular_velocity: {desired_angular_velocity[indices]}")
  # print(f"base_angular_velocity_body_frame: {base_angular_velocity_body_frame[indices]}")
  # print(f"base angular velocity: {torch.matmul(base_rot_mat, base_angular_velocity_body_frame[:, :, None])[:, :, 0][indices]}")
  # print(f"angular vel error: {ang_vel_error[indices]}")
  # print(f"Desired lin vel: {desired_linear_velocity}")
  # print(f"Current lin vel: {torch.matmul(base_rot_mat, base_velocity_body_frame[:, :, None])[:, :, 0]}")
  # print(f"Desired lin acc body: {desired_lin_acc_body_frame[indices]}")
  # print(f"Desired ang acc body: {desired_ang_acc_body_frame[indices]}")
  # print("lin_vel_error:\n")
  #input("Any Key...")
...
def construct_mass_mat(foot_positions,
  ...
  mass_mat[:, 3:6] = torch.matmul(inv_inertia.float(), px.float())
...
class QPTorqueOptimizer:
  ...
  def __init__(self,
              ...
              #base_position_kp=np.array([0., 0., 1600]),
                base_position_kd=np.array([10., 10., 10.]),
              #base_position_kd=np.array([2000., 2000., 4000.]),
                base_orientation_kp=np.array([50., 50., 0.]),
              # base_orientation_kp=np.array([600., 600., 0.]),
               base_orientation_kd=np.array([10., 10., 10.]),
               #base_orientation_kd=np.array([0., 0., 500.]),
              weight_ddq=np.diag([1., 1., 100., 10., 10., 1.]),
              # weight_ddq=np.diag([1., 1., 1., 20., 20., 10.]),
    ...
    self._base_orientation_kp = convert_to_torch(base_orientation_kp,
                                         device=self._device)
    self._base_orientation_kp = torch.stack([self._base_orientation_kp] *
                                            self._num_envs,
                                            dim=0)
    self._base_orientation_kd = convert_to_torch(base_orientation_kd,
                                         device=self._device)
    self._base_orientation_kd = torch.stack([self._base_orientation_kd] *
                                            self._num_envs,
                                            dim=0)
    self._base_position_kp = convert_to_torch(base_position_kp, device=self._device)
    self._base_position_kp = torch.stack([self._base_position_kp] *
                                         self._num_envs,
                                         dim=0)
    self._base_position_kd = convert_to_torch(base_position_kd, device=self._device)
    ...
    self._desired_angular_acceleration = torch.zeros((self._num_envs, 3),
                                                     device=self._device)
    self._Wq = convert_to_torch(weight_ddq, device=self._device, dtype=torch.float32)
    self._Wf = torch.tensor(weight_grf, device=self._device)
    ...
    self._inv_inertia = torch.linalg.inv(
        torch.diag(convert_to_torch(body_inertia, device=self._device)))

  def _solve_joint_torques(self, foot_contact_state, desired_com_ddq):
    """Solves centroidal QP to find desired joint torques."""
    # print("Foot Contact State: ", foot_contact_state)
    # print("foot_positions_in_base_frame: ", self._robot.foot_positions_in_base_frame)
  ...
  def compute_joint_command(self, foot_contact_state: torch.Tensor,
    ...
    desired_acc_body_frame = torch.clip(
        ...
        convert_to_torch([-30, -30, -10, -20, -20, -20], device=self._device),
        convert_to_torch([30, 30, 30, 20, 20, 20], device=self._device))
    ...
    # print("Current Foot Position: {}".format(foot_position_local))
    # print("Desired Foot Position: {}".format(desired_foot_position))
    # print("Current motor position: {}".format(self._robot.motor_positions))
    # print("Desired motor position: {}".format(desired_motor_position))
    # # print("Current Base Position: {}".format(self._robot.base_position))
    # # print("Desired Base Position: {}".format(desired_base_position))
    # print("Current Base Orientation: {}".format(self._robot.base_orientation_rpy))
    # print("Desired Base Orientation: {}".format(desired_base_orientation_rpy))
    # print("Current Base Velocity: {}".format(self._robot.base_velocity_body_frame))
    # print("Desired Base Velocity: {}".format(desired_linear_velocity))
    # print("Current Base Angular Velocity: {}".format(self._robot.base_angular_velocity_body_frame))
    # print("Desired Base Angular Velocity: {}".format(desired_angular_velocity))
  ...
  def desired_base_position(self, base_position: float):
    self._desired_base_position = convert_to_torch(base_position, device=self._device)
  ...
  def desired_base_orientation_rpy(self, orientation_rpy: torch.Tensor):
    self._desired_base_orientation_rpy = convert_to_torch(orientation_rpy,
                                                  device=self._device)
  ...
  def desired_linear_velocity(self, desired_linear_velocity: torch.Tensor):
    self._desired_linear_velocity = convert_to_torch(desired_linear_velocity,
                                             device=self._device)
  ...
  def desired_angular_velocity(self, desired_angular_velocity: torch.Tensor):
    self._desired_angular_velocity = convert_to_torch(desired_angular_velocity,
                                              device=self._device)
  ...
  def desired_linear_acceleration(self,
                                  desired_linear_acceleration: torch.Tensor):
    self._desired_linear_acceleration = convert_to_torch(desired_linear_acceleration,
                                                 device=self._device)
  ...
  def desired_angular_acceleration(self,
                                   desired_angular_acceleration: torch.Tensor):
    self._desired_angular_acceleration = convert_to_torch(desired_angular_acceleration,
                                                  device=self._device)
