...
import torch
import numpy as np
from sep_vae_dyn import TrajectoryDataset, LatentDynamicsLearner, LatentDynamicsModel, LatentLearnerCfg
...
def compute_k_step_errors(
    ...
    with torch.no_grad():
        for batch in learner.eval_dyn_dataloader:
            ...
            batch_size, seq_len = batch['valid_mask'].shape
            ...
            z = model.latent(obs) # N x latent_dim
            pred_curr_obs = model.predictor.forward(z) # N x 1 x obs_dim
            pred_next_obs = [pred_curr_obs]
            current_z = z
            for i in range(k):
                current_z = model.next_latent(current_z, actions[:, i])
                obs_pred = model.predictor.forward(current_z)
            ...
            for key in obs_keys:
                ...
                rel_error = abs_error / (true_key.abs() + 1e-6)        # [N, k, dim]

                # Reduce across the last dimension to get scalar error for each step
    ...
    for key in obs_keys:
        ...
        results[key] = {
            ...
            'rel_norms': np.mean(rel_norm, axis=0),
            'abs_std': np.std(abs_norm, axis=0),
            'rel_std': np.std(rel_norm, axis=0),
            'abs_lower': np.quantile(abs_norm, quantiles[0], axis=0),
            'abs_upper': np.quantile(abs_norm, quantiles[1], axis=0),
            'rel_lower': np.quantile(rel_norm, quantiles[0], axis=0),
            'rel_upper': np.quantile(rel_norm, quantiles[1], axis=0),
        }
...
if __name__ == "__main__":
    model_path = 'logs/test_separate/20250326-194058/model_80.pt'
    fig_prefix = 'GRU128_real_dataset_quantile2575'
    dataset_path = 'episodes_states_real_23dof.npy'
    variance_type = 'quantile'    # quantile or std
