...
#############################################
KEY_DIMENSIONS = {
    "base_pos": {'dim':1, 'idx_start':0, 'range': torch.tensor([1.2])},         # z coord position
    "base_lin_vel": {'dim':3, 'idx_start':1, 'range': torch.tensor([10.0]*3)},      
    "base_ang_vel": {'dim':3, 'idx_start':4, 'range': torch.tensor([10.0]*3)},           
    "base_quat": {'dim':4, 'idx_start':7, 'range': torch.tensor([1.0]*4)},             
    "joint_pos": {'dim':23, 'idx_start':11, 'range': torch.tensor([1.0]*23)},            
}
...
class LatentDynamicsLearner:
    ...
    def postprocess_results(self, results, model):
        ...
        if not self.is_tuning and hasattr(self, 'writer'):
            ...
            for k in required_keys:
                self.writer.add_scalar(f"{model}/Train/{k}_ave", out_dict[f"train_{k}_ave"], ep)
                self.writer.add_scalar(f"{model}/Train/{k}_std", out_dict[f"train_{k}_std"], ep)
                self.writer.add_scalar(f"{model}/Eval/{k}_ave",  out_dict[f"eval_{k}_ave"],  ep)
                self.writer.add_scalar(f"{model}/Eval/{k}_std",  out_dict[f"eval_{k}_std"],  ep)
...
class LatentLearnerCfg:
    ...
    eval_pct = 0.2
    logger = "tensorboard"
    log_dir = "./logs/tune_VAE_dyn_separate"  # base log directory
    save_interval = 10
    eval_interval = 2
    seq_len = 12
    batch_size = 64
